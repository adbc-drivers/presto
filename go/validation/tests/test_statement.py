...
def test_query_id_query(driver, conn) -> None:
    with conn.cursor() as cursor:
        query_id = cursor.adbc_statement.get_option("presto.statement.last_query_id")
        assert query_id == ""
...
def test_query_id_query_bind(driver, conn) -> None:
    with conn.cursor() as cursor:
        ...
        with pyarrow.RecordBatchReader._import_from_c(stream.address) as reader:
            for batch in reader:
                seen.add(
                    cursor.adbc_statement.get_option("presto.statement.last_query_id")
                )
